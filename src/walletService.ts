...
export async function connectMetaMask(): Promise<ConnectedWallet> {
  if (typeof window === 'undefined' || !window.ethereum) {
    throw new Error('MetaMask or Web3 wallet not detected. Please install MetaMask or open this app inside your Web3 wallet browser (MetaMask, Coinbase Wallet, Trust Wallet).');
  }
  ...
  try {
    ...
    if (error.code === 4001 || error.message?.includes('User rejected')) {
      throw new Error('Transaction/Connection was rejected by user in wallet.');
    }
    throw new Error(error.message || 'Failed to connect Web3 wallet');
  }
}
...
export async function connectPhantom(): Promise<ConnectedWallet> {
  ...
  if (!provider) {
    throw new Error('Phantom wallet not detected. Please install Phantom from phantom.app or open this app inside your Phantom mobile browser.');
  }
  ...
  try {
    ...
    try {
      ...
      console.warn('Solana RPC query error:', e);
    }
    ...
    if (error.code === 4001 || error.message?.includes('User rejected')) {
      throw new Error('Connection rejected by user in Phantom.');
    }
...
export async function deployNFTContract(
  ...
  if (wallet.network === 'ethereum' && window.ethereum) {
    ...
    const tx = new solanaWeb3.Transaction({
      feePayer: new solanaWeb3.PublicKey(wallet.address),
      blockhash,
